// ** SETUP **
// Assign the BasicFPCC object to its own Layer (if you haven't already, create a new Layer called Player and assign the empty GameObject BasicFPCC to it)
// Assign the Layer Mask to ignore the BasicFPCC object Layer (in the Inspector for BasicFPCC, select the Casting Mask drop down in the Basic FPCC (Script) component and select everything except for Player)
...
public class BasicFPCC : MonoBehaviour
{
    ...
    public float slideDuration = 1.1f;               // duration of slide
    public float gravity = -9.81f;                   // gravity / fall rate
    public float jumpHeight = 1.8f;                  // jump height
    ...
    void Initialize()
    {
        ...
        cam = cameraTx.GetComponent<Camera>();
    ...
    void ProcessMovement()
    {
        ...
        //FUNCTIONALITY FOR RUNNING / SPRINTING
        ...
        if (isSprinting)
        {
            if (cam.fieldOfView < initFOV * 1.1f)
            {
                cam.fieldOfView += 1;
            }
        }
        else if (cam.fieldOfView > initFOV)
        {
            cam.fieldOfView -= 0.5f;
        }


        //FUNCTIONALITY FOR CROUCHING -- COMMENTED OUT DUE TO: Unused
        /*
        ...
        */
        ...
        //FUNCTIONALITY FOR SLIDING -- COMMENTED OUT DUE TO: Unused
        /*
        ...
        if ( isSliding )
        {
            ...
            if ( slideTimer > slideDuration || currSpeed < crouchSpeed || inputKeyDownJump || !inputKeyRun)
            {
                isSliding = false;
            }
        ...
        if ( isGrounded )
        {
            ...
            // - Jump -
            //FUNCTIONALITY FOR JUMPING -- COMMENTED OUT DUE TO: Unused
            /*(
